...
def copy_starter_files():
    return [
        ...
        path_is_not_file(vr('FileTgt')), and_(),
...
def declare_for_bootstrap():
    return [
        ...
        path_is_not_file(vr('Script')), and_(),
        echo_fatal(['Missing script ', sq(vr('Script'))]), and_(),
        return_(63), eol(),
        source(dq(vr('Script'))), seq(),
        assign(vn('Status'), '$?'), eol(),
        '[[ ', dq(vr('Status')), ' -ne 0 ]]', and_(),
